...
type (
	idSenderModel interface {
		...
		FindOneByBizTag( bizTag string) (*IdSender, error)
		Update(ctx context.Context, data *IdSender) error
		Delete(ctx context.Context, id int64) error
		UpdateCurrentIdAutoStep( bizTag string) error
	}
...
func (m *defaultIdSenderModel) FindOneByBizTag(bizTag string) (*IdSender, error) {
	var resp IdSender
	query := fmt.Sprintf("select %s from %s where `biz_tag` = ? limit 1", idSenderRows, m.table)
	err := m.conn.QueryRowCtx(nil, &resp, query, bizTag)
	switch err {
	case nil:
		return &resp, nil
	case sqlx.ErrNotFound:
...
func (m *defaultIdSenderModel) Update(ctx context.Context, newData *IdSender) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, idSenderRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.BizTag, newData.CurrentId, newData.Step, newData.Id)
	return err
}
func (m *defaultIdSenderModel) UpdateCurrentIdAutoStep( bizTag string) error {
	query := fmt.Sprintf(`
        UPDATE %s 
        SET current_id = current_id + step, update_time = CURRENT_TIMESTAMP 
        WHERE biz_tag = ?`, m.table)

	_, err := m.conn.ExecCtx(nil, query, bizTag)
	return err
}
